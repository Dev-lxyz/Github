    ...
    try {
      ...
      if (!uid) {
        return res.status(400).json({
          status: false,
          message: "Falta el parámetro uid"
        })
      }

      const response = await fetch(
        `https://www.00cc.eu.cc/freefire-stalk?uid=${encodeURIComponent(uid)}`,
        {
          headers: {
      ...
      res.json({
        ...
        data: {
          ...
          overview: {
            ...
            weapon_skin_shows: r.account_overview.weapon_skin_shows?.map(v => v.name) || [],
            equipped_skills: r.account_overview.equipped_skills?.map(v => v.name) || []
          },

          equip: {
            profile: r.equip_items.profile?.map(v => ({
              name: v.name,
              type: v.type,
              rare: v.rare
            })) || [],

            character: r.equip_items.character?.map(v => ({
              name: v.name,
              type: v.type,
              rare: v.rare
            })) || [],

            outfit: r.equip_items.outfit?.map(v => ({
              name: v.name,
              type: v.type,
              rare: v.rare
            })) || [],

            weapon: r.equip_items.weapon?.map(v => ({
              name: v.name,
              type: v.type,
              rare: v.rare
            })) || [],

            pet: r.equip_items.pet?.map(v => ({
              name: v.name,
              type: v.type,
              rare: v.rare
            })) || []
