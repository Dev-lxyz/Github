module.exports = function (app) {
  ...
  async function searchRaw(term, limit) {
    ...
    const tryHashes = async (hashes) => {
      for (const hash of hashes) {
        ...
        if (json?.data?.searchV2) return json
      }
      return null
    }
    const primary = discoveredHash ? [discoveredHash, ...FALLBACK_HASHES] : FALLBACK_HASHES
    let json = await tryHashes(primary)
    if (!json) {
      const fresh = await discoverHash()
      if (fresh && !primary.includes(fresh)) json = await tryHashes([fresh])
    }
    return json
  }

  // Endpoint debug: devuelve el 
  app.get('/debug/spotify-raw', async (req, res) => {
    try {
      ...
      const lim = Math.min(Math.max(parseInt(limit) || 3, 1), 10)
      const raw = await searchRaw(term, lim)

      if (!raw) {
        return res.json({
          status: false,
          error: "Spotify no devolvio datos (revisa hash/token)"
        })
      }

      // Esto es TODO lo que Spotify manda, sin tocar nada
      res.json(raw)
