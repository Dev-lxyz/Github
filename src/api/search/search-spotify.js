module.exports = function (app) {
  ...
  // Cache simple en memoria: term+limit -> { data, expiresAt }
  ...
  function parseTrack(d) {
    ...
    return {
      ...
      title: d.name || null,
      album: d.albumOfTrack?.name || null,
  ...
  async function spotifySearch(searchTerm, limit) {
    ...
    try {
      ...
      // Previews en paralelo: con limit bajo (ej. 1) esto resuelve casi instantaneo
  ...
  app.get('/search/spotify', async (req, res) => {
    try {
      ...
      const lim = Math.min(Math.max(parseInt(limit) || 1, 1), 10)
      const cacheKey = `${term.toLowerCase().trim()}|${lim}`
      ...
      const result = await spotifySearch(term, lim)
